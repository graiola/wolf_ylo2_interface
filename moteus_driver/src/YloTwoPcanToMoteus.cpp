...
YloTwoPcanToMoteus::YloTwoPcanToMoteus()
{
  ...
  _stop.MSGTYPE = PCAN_MESSAGE_STANDARD | PCAN_MESSAGE_BRS | PCAN_MESSAGE_FD;
  _stop.DLC = 5; // 5 bytes
  // Write Mode
...
bool YloTwoPcanToMoteus::send_commands(int id, int port, float fftorque){
    ...
    std::cout << "erreur dans fonction send_commands(), id=" << id << ". Status = " << strMsg << std::endl;
    return(false);
}
